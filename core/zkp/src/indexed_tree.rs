...
pub(crate) fn hash_leaf_native<F: PrimeField + Absorb>(leaf: &IndexedLeaf<F>) -> F {
...
pub fn build_indexed_tree<F: PrimeField + Absorb>(leaves: &[IndexedLeaf<F>]) -> Vec<Vec<F>> {
...
#[derive(Clone)]
pub struct SparseIndexedTree<F: PrimeField + Absorb> {
    depth: usize,
    default_hashes: Vec<F>,
    nodes: Vec<HashMap<usize, F>>,
}

impl<F: PrimeField + Absorb> SparseIndexedTree<F> {
