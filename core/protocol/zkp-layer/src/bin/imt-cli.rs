...
use imt_non_membership::{
    generate_manager_keypair, pack_receipt, public_key_to_fields, IndexedBatchInsertCircuit,
    IndexedInsertStep, IndexedLeaf, ManagerKeypair, TaskReceipt, SparseIndexedTree,
};

#[derive(Parser)]
#[command(author, version, about = "Utility CLI for BN254 indexed insert prover setup")]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
...
fn setup_command(
    ...
    let pk: ProvingKey<Bn254> = Groth16::<Bn254, LibsnarkReduction>::
        generate_random_parameters_with_reduction(circuit.clone(), &mut rng)?;
    let vk = pk.vk.clone();
