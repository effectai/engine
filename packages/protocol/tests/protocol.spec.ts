	...
	describe("Libp2p: Effect AI Protocol", () => {
		...
		it(
			...
			async () => {
				...
				await manager1.services.manager.checkChallenges();

				await new Promise((resolve) => setTimeout(resolve, 1000));
				//get challenge created by manager
				const [challenge] = await w1.services.worker.getChallenges();
				await w1.services.worker.completeChallenge(
					challenge.id,
					`{"worker":"${w1.peerId.toString()}", "result": "7"}`,
				);
				await manager1.services.manager.checkChallenges();
