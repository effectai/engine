...
export interface Libp2pMethods {
  ...
  ): Promise<[MessageResponse<T> | null, EffectProtocolError | null]>;
  ...
  private async sendMessage<T extends EffectProtocolMessage>(
    ...
    try {
      ...
      if (response.error) {
        return [
          null,
          new EffectProtocolError(response.error.code, response.error.message),
        ];
      }
      ...
      if (error instanceof Error) {
        const protocolError =
          error instanceof EffectProtocolError
            ? error
            : new EffectProtocolError("NETWORK_ERROR", error.message);
        return [null, protocolError];
      }

      return [null, new EffectProtocolError("UNKNOWN_ERROR", "Unknown error")];
    } finally {
