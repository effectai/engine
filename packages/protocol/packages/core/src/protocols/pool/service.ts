...
import { Pool, Type } from "./pb/workerPool.js";
	...
	implements Startable
{
	...
	afterStart() {
		...
		this.components.pubsub.subscribe("effectai-worker-pool");
		this.components.pubsub.addEventListener("message", ({ detail }) => {
			...
			try {
				const peer = Pool.decode(detail.data);
	...
	stop() {
		...
		if (this.components.pubsub) {
			this.components.pubsub?.unsubscribe("effectai-worker-pool");
	...
	_broadcast() {
		...
		const encodedPeer = Pool.encode(peer);
		...
		if (pubsub.getSubscribers("effectai-worker-pool").length === 0) {
			// console.warn("No subscribers for effectai-worker-pool");
			return;
		}

		void pubsub.publish("effectai-worker-pool", encodedPeer);
	}
}
