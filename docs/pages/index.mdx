...
1. **Safety**. AI agents are often driven by statistical models with an imperfect representation of our world. Many real-world applications can not operate on the uncertainty that these models bring, and require validation. 
2. **Fine-tuning** and **Prompting**. Domain experts are able to improve the output of AI models by fine tuning them on specific datasets and using specific ways of prompting.
3. **Embodiment**. Agents are not able to operate in the real world - they do not own credit cards, do not own hardware, and are not citizens. Human input is required for agents to operate and to realize goals in the physical world.
