    ...
    describe("Migration Contract: Signing & Claiming", () => {
      it("correctly claims with an eos signature", async () => {
        ...
        await program.methods
          .claim(Buffer.from(signature.data.array), Buffer.from(message), false)
          .accounts({
      ...
      it("correctly signs with keccak256", async () => {
        ...
        await program.methods
          .claim(sigWithRecovery, Buffer.from(message), true)
          .accounts({
            metadataAccount: metadata,
            recipientTokens: ata,
            payer: payer.publicKey,
      ...
      it("correctly claims with eth_personal_sign", async () => {
        ...
        await program.methods
          .claim(Buffer.from(toBytes(signature)), Buffer.from(message), true)
          .accounts({
            recipientTokens: ata,
            metadataAccount: metadata,
            vaultAccount: vault,
