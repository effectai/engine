...
> = {
  ...
  handler: async ({ mint, distribution_file }) => {
    ...
    const confirmed = await askForConfirmation(
      `Distribute (${claimsLeft.length}/${rows.length}) claims on ${
      ...
      try {
        ...
        if (e instanceof Error) {
          if (
            ...
            e.message.includes("Instruction: CreateStakeClaim") &&
            ...
            if (confirmed) {
              ...
              try {
                ...
                row.tx = await provider.sendAndConfirm(topupTransaction, [
                  payer,
                ]);
                row.topup = true;
