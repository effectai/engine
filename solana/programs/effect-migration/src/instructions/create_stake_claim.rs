...
use crate::ClaimStakeAccount;

#[derive(Accounts)]
#[instruction(foreign_public_key: Vec<u8>)]
pub struct CreateStakeClaim<'info> {
    #[account(
        init,
        payer = payer,
        space = 8 + 32,
...
impl<'info> CreateStakeClaim<'info> {
...
pub fn handler(ctx: Context<CreateStakeClaim>, foreign_public_key: Vec<u8>, stake_start_time: i64, amount: u64) -> Result<()> {
    ...
    let (vault_authority, _) = Pubkey::find_program_address(
        &[ctx.accounts.claim_account.to_account_info().key.as_ref()],
        ctx.program_id,
    );
