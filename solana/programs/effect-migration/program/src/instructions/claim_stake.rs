...
use effect_migration_common::{ClaimType, MigrationAccount, MigrationProgram};
...
impl<'info> ClaimStake<'info> {
    fn into_genesis_stake_context(&self) -> CpiContext<'_, '_, '_, 'info, GenesisStake<'info>> {
        let cpi_accounts = GenesisStake {
            ...
            stake_account: self.stake_account.to_account_info().clone(),
            ...
            migration_program: self.migration_program.to_account_info().clone(),
